...
An administrative Discord bot powered by Anthropic Claude models with a dual-agent safety layer. AI1 plans and converses naturally; AI2 reviews planned scripts and escalates risky actions for manual Telegram approval.

## Features
- Conversational AI (Claude haiku models) that can understand mixed chit-chat plus embedded administrative requests.
- Structured AI1 outputs: human reply plus runnable Python action scripts.
- AI2 reviewer to auto-approve safe scripts or escalate higher-risk actions to a Telegram bot with human-in-the-loop controls.
- Restricted Python executor without filesystem access, using only approved Discord helper functions.
...
1. **AI1 conversation**: AI1 receives the admin message plus one-hour conversational memory and recent actions. It returns JSON with a human-facing reply and optional Python scripts implementing the requested actions.
2. **AI2 review**: AI2 heuristically inspects each script for risky patterns (external calls, timed execution, multi-user impact, or large size). Safe scripts run immediately; risky ones are escalated.
...
4. **Execution sandbox**: Approved scripts run in a constrained executor exposing only Discord helper functions (`send_message`, `create_role`, `assign_role`, `fetch_history`) with limited built-ins—no filesystem access or secret exposure.
